...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        addCommentMutation(state, data) {

            const updateIndex = state.messages.findIndex(item => item.id === data.message.id)
            ...
            if(message.comments === null){
                ...
                if (!message.comments.find(it => it.id === data.id)) {
                    state.messages = [
                        ...state.messages.slice(0, updateIndex),
                        {
                            ...message,
    ...
    actions: {
        async addMessageAction({commit, state}, message) {
            ...
            const index = await state.messages.findIndex(item => item.id === data.id)
            if (index > -1) {
                await commit('updateMessageMutation', data)
            } else {
                await commit('addMessageMutation', data)
            }
        },
